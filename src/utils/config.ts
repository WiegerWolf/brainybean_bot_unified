...
class Config {
    ...
    try {
      ...
      if (err instanceof z.ZodError) {
        const msgs = err.issues
          ...
          .join("; ");
        throw new Error(
          `Invalid environment configuration: ${msgs}. Copy .env.example to .env and set required variables like TELEGRAM_BOT_TOKEN and OPENAI_API_KEY.`
        );
      }
      throw err;
    }
  ...
  get TELEGRAM_BOT_TOKEN() {
    return this.data.TELEGRAM_BOT_TOKEN;
  }
  get OPENAI_API_KEY() {
